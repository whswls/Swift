...
struct SampleJournalEntryData {
    ...
    mutating func createSampleJournalEntryData() {
        ...
        guard let journalEntry1 = JournalEntry(rating: 1, title: "Good", body: "Today is a good day", photo: photo1) else {
            fatalError("Unable to instantiate journalEntry1")
        }
        guard let journalEntry2 = JournalEntry(rating: 2, title: "Average", body: "Today was okay", photo: photo2) else {
            fatalError("Unable to instantiate journalEntry2")
        }
        guard let journalEntry3 = JournalEntry(rating: 3, title: "Bad", body: "Today was terrible", photo: photo3) else {
